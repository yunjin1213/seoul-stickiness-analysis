...
def build_dominant_time_lookup(rows):
    ...
    lookup = {}
    for dong_code, item in best_by_dong.items():
        lookup[dong_code] = item[1]
    return lookup
...
def main():
    ...
    write_rows(
        os.path.join(results_dir, "question_answer_evidence.csv"),
        EVIDENCE_COLUMNS,
        evidence_rows,
    )
    write_rows(
        os.path.join(results_dir, "result_interpretation_support.csv"),
        SUPPORT_COLUMNS,
